...
extern DLLEXPORT void ri_param_eval(void                 *dst,
			            const ri_parameter_t *param,
				    const char           *name);
extern DLLEXPORT void ri_param_add(ri_parameter_t *param,
			 	   const char     *name,
				   int             type,
				   const void     *val);
extern DLLEXPORT void ri_param_override(ri_parameter_t *param,
					const char     *name,
					const void     *val);

extern void ri_vector_reflect(ri_vector_t *dst, const ri_vector_t *in, const ri_vector_t *n);
...
extern DLLEXPORT void faceforward(ri_vector_t *dst,
			const ri_vector_t *N, const ri_vector_t *I);
extern DLLEXPORT void normalize(ri_vector_t *dst, const ri_vector_t *N);
extern DLLEXPORT void reflect(ri_vector_t *dst,
		              const ri_vector_t *I, const ri_vector_t *N);
extern DLLEXPORT void refract(ri_vector_t *dst,
		              const ri_vector_t *I, const ri_vector_t *N,
                              float eta);
extern DLLEXPORT void transform(ri_vector_t *dst,
		      const char *tospace, const ri_vector_t *src);
extern DLLEXPORT void vtransform(ri_vector_t *dst,
		       const char *from, const char *to, const ri_vector_t *src);

/* shading functions. all shading function needs ri_status_t argument. */
extern DLLEXPORT void ambient(const ri_status_t *status,
			      ri_color_t *dst);
extern DLLEXPORT void diffuse(const ri_status_t *status,
                              ri_color_t *dst, const ri_vector_t *N);
extern DLLEXPORT void specular(const ri_status_t *status,
                               ri_color_t *dst,
		               const ri_vector_t *N, const ri_vector_t *V,
		               float roughness);
extern DLLEXPORT void texture(const ri_status_t *status,
                              ri_color_t *dst, const char *name);

/* texture() with coordinates */
extern DLLEXPORT void texture_coords(const ri_status_t *status,
				     ri_color_t *dst,
			             const char *name,
                                     const ri_vector_t *coords);
extern DLLEXPORT void environment(const ri_status_t *status,
                                  ri_color_t *dst,
			          const char *name, const ri_vector_t *coords);

extern DLLEXPORT float occlusion(const ri_status_t *status,
                                 const ri_vector_t *P, const ri_vector_t *N,
                                 float nsamples);
extern DLLEXPORT void trace(const ri_status_t *status,
                            ri_vector_t *dst,
                            const ri_vector_t *P, const ri_vector_t *R);
...
extern DLLEXPORT void  mixv(ri_vector_t *dst,
                            const ri_vector_t *x, const ri_vector_t *y,
                            float alpha);

/* geometric functions */
extern DLLEXPORT float area(const ri_vector_t *P);
extern DLLEXPORT float distant(const ri_vector_t *p1, const ri_vector_t *p2);
...
extern DLLEXPORT ri_lightsource_t *next_lightsource(const ri_status_t *status,
						    const ri_vector_t *P,
						    const ri_vector_t *N,
					            float angle);
