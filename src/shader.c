...
static int typesize[] = {sizeof(float) * 4, sizeof(char), sizeof(float)};
...
static void gen_ortho_basis(ri_vector_t *basis, const ri_vector_t *n);
static void status_copy(ri_status_t *dst, const ri_status_t *src);
...
void
ri_vector_reflect(ri_vector_t *dst, const ri_vector_t *in, const ri_vector_t *n)
{
	double dot;

	dot = ri_vector_dot(in, n);

	dst->e[0] = (float)(in->e[0] - 2.0 * dot * n->e[0]);
	dst->e[1] = (float)(in->e[1] - 2.0 * dot * n->e[1]);
	dst->e[2] = (float)(in->e[2] - 2.0 * dot * n->e[2]);
}
...
void
faceforward(ri_vector_t *dst, const ri_vector_t *N, const ri_vector_t *I)
{
	double dot;
	ri_vector_t eye;
	...
	if (dot < -2.0e-1) {		/* consider machine epsilon */
		dst->e[0] = -dst->e[0];
		dst->e[1] = -dst->e[1];
		dst->e[2] = -dst->e[2];
	}
...
void
normalize(ri_vector_t *dst, const ri_vector_t *N)
{
	ri_vector_copy(dst, N);
...
void
ambient(const ri_status_t *status, ri_color_t *dst)
{
	ri_vector_t ambcol;
...
void
diffuse(const ri_status_t *status, ri_color_t *dst, const ri_vector_t *N)
{
	double      dot;
	ri_vector_t light;
...
void
specular(const ri_status_t *status,
         ri_color_t *dst, const ri_vector_t *N, const ri_vector_t *V,
	 float roughness)
{
	double      dot;
	double      coeff;
...
void
reflect(ri_vector_t *dst, const ri_vector_t *I, const ri_vector_t *N)
{
	ri_vector_reflect(dst, I, N);
}

void
refract(ri_vector_t *dst, const ri_vector_t *I, const ri_vector_t *N, float eta)
{
	ri_refract(dst, I, N, eta);
}

void
transform(ri_vector_t *dst, const char *tospace, const ri_vector_t *src)
{
...
void
vtransform(ri_vector_t *dst, const char *from, const char *to,
...
void
environment(const ri_status_t *status,
            ri_color_t *dst, const char *name, const ri_vector_t *coords)
{
	double      m;
	ri_vector_t tex_coords;

	m = 2.0 * sqrt(coords->e[0] * coords->e[0] +
		       coords->e[1] * coords->e[1] +
		       (coords->e[2] + 1.0) * (coords->e[2] + 1.0));

	
	if (m != 0.0) {
		tex_coords.e[0] = coords->e[0] / m + 0.5;
		tex_coords.e[1] = coords->e[1] / m + 0.5;
	} else {
		tex_coords.e[0] = 0.5;
		tex_coords.e[1] = 0.5;
	}
...
void
texture(const ri_status_t *status, ri_color_t *dst, const char *name)
{
...
void
texture_coords(const ri_status_t *status,
	       ri_color_t *dst, const char *name, const ri_vector_t *coords)
{
	const int step = 0x8;
	...
	u = (int)(texwidth * coords->e[0]) % texwidth;
	v = (int)(texwidth * coords->e[1]) % texwidth;
...
float
occlusion(const ri_status_t *status,
          const ri_vector_t *P, const ri_vector_t *N, float nsamples)
{
	int               i, j, k;
	int               hit;
	int               coverage;
	...
	for (j = 0; j < nphi; j++) {
		for (i = 0; i < (int)ntheta; i++) {
			...
			for (k = 0; k < 3; k++) {
				ray.dir.e[k] = dir.e[0] * basis[0].e[k]
					     + dir.e[1] * basis[1].e[k]		
					     + dir.e[2] * basis[2].e[k];
			}
			...
			// the ray direction
			ray.org.e[0] += 0.0001 * ray.dir.e[0]; 
			ray.org.e[1] += 0.0001 * ray.dir.e[1]; 
			ray.org.e[2] += 0.0001 * ray.dir.e[2]; 
	...
	return (float)coverage / (float)nsamples;
}
...
void
trace(const ri_status_t *status,
      ri_vector_t *dst, const ri_vector_t *P, const ri_vector_t *R)
{
	int          hit;
	ri_vector_t  eye;
	...
	ri_vector_sub(&eye, &surfinfo.pos, P);
	...
	ri_vector_copy(&newstatus.input.Cs,   &surfinfo.color);
	ri_vector_copy(&newstatus.input.P,    &surfinfo.pos);
	ri_vector_copy(&newstatus.input.N,    &surfinfo.normal);
	ri_vector_copy(&newstatus.input.dPdu, &surfinfo.tangent);
	ri_vector_copy(&newstatus.input.dPdv, &surfinfo.binormal);
	ri_vector_copy(&newstatus.input.I,    &eye);
	newstatus.input.s = surfinfo.u;
	newstatus.input.t = surfinfo.v;

	opa = surfinfo.opacity;
	...
	//ri_vector_copy(dst, &surfinfo.color);
}
...
void
mixv(ri_vector_t *dst, const ri_vector_t *x, const ri_vector_t *y, float alpha)
{
	dst->e[0] = (1.0 - alpha) * x->e[0] + alpha * y->e[0];
	dst->e[1] = (1.0 - alpha) * x->e[1] + alpha * y->e[1];
	dst->e[2] = (1.0 - alpha) * x->e[2] + alpha * y->e[2];
}

float
distant(const ri_vector_t *p1, const ri_vector_t *p2)
{
	ri_vector_t d;
		 ...
		 const ri_vector_t *P, const ri_vector_t *N, float angle)
{
	...
	ri_vector_copy(&ray.org, P);
	ray.org.e[0] += N->e[0] * 0.0001;	
	ray.org.e[1] += N->e[1] * 0.0001;	
	ray.org.e[2] += N->e[2] * 0.0001;	

	ray.thread_num = tid;

	if (glightinfo[tid].light->ibl) {
...
static void
gen_ortho_basis(ri_vector_t *basis, const ri_vector_t *n)
{
	int i;

	ri_vector_copy(&basis[2], n);
	basis[1].e[0] = basis[1].e[1] = basis[1].e[2] = 0.0;

	for (i = 0; i < 3; i++) {
		if (basis[2].e[i] < 0.6 && basis[2].e[i] > -0.6) break;
	}
...
get_light(ri_render_t *render)
{
	...
	if (!ri_list_first(render->lightlist)) {
		//ri_log("warning", "no light exist.\n");
		return NULL;
	}

	light = (ri_light_t *)ri_list_first(render->lightlist)->data;

	return light;
}

static void
init_lightsource(const ri_status_t *status,
		 const ri_vector_t *P, const ri_vector_t *N, float angle)
{
	int          i, j, k;
	int          count = 0;
	...
	for (j = 0; j < nphi; j++) {
		for (i = 0; i < ntheta; i++) {
			...
			//assert(!isnan(dir.e[0]));
			//assert(!isnan(dir.e[1]));
			//assert(!isnan(dir.e[2]));

			for (k = 0; k < 3; k++) {
				ldir.e[k] = dir.e[0] * glightinfo[tid].basis[0].e[k]
				          + dir.e[1] * glightinfo[tid].basis[1].e[k]
				          + dir.e[2] * glightinfo[tid].basis[2].e[k];
			}
			ldir.e[3] = 0.0;

			ri_vector_normalize(&ldir);

			ri_vector_copy(&(glightinfo[tid].samples[count].L),
				       &ldir);

			/* get light color from IBL image */
			ri_texture_ibl_fetch(&(glightinfo[tid].samples[count].Cl),
					     glightinfo[tid].light->texture,
					     &ldir);

#if defined(LINUX) || defined(__APPLE__)
			if (isnan(glightinfo[tid].samples[count].Cl.e[0])) {
				printf("ldir = ");
			...
			ri_vector_scale(&(glightinfo[tid].samples[count].Cl),
					1.0f / (float)glightinfo[tid].nsamples);

			/* Ol is not yet implemented */
			ri_vector_zero(&glightinfo[tid].samples[count].Ol);
