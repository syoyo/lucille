...
static ri_hash_node_t **hash_node_lookup(ri_hash_t *hash,
                     const char *key);
static void            hash_resize     (ri_hash_t *hash);
...
hash_node_lookup(ri_hash_t *hash, const char *key)
{
    unsigned int     index;
    int              len;
    ri_hash_node_t **p;
...
hash_resize(ri_hash_t *hash)
{
    ...
    for (i = 0; i < hash->size; i++) {
        for (p = hash->nodes[i]; p != NULL; p = next) {
            ...
            hashval = hash_func(p->key) % newsize;
