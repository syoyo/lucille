	...
	for (i = 0; i < n; i++) {
		...
		if (strcmp(tokens[i], RI_P) == 0) { /* position */
			...
			for (j = 0; j < nverts; j++) {
				 ...
				 * to world space
				 */
				ri_vector_set_rman(&v, &param[3 * j]);
			...
			for (j = 0; j < nverts; j++) {
				...
				ri_vector_normalize3(&normals[j]);
				if(two_sided) {
			...
			for (j = 0; j < nverts; j++) {
				ri_vector_set_rman(&(colors[j]), &param[3 * j]);
				if (two_sided) {
	...
	for (i = 0; i < (unsigned int)n; i++) {
		...
		if (strcmp(tokens[i], RI_P) == 0) { /* position */
			...
			for (j = 0; j < nvertices; j++) {
				 ...
				 * to world space
				 */
				ri_vector_set_rman(&v, &param[j * 3]);
				ri_vector_transform(&positions[j], &v, &om);
...
calc_area(const ri_vector_t *v0, const ri_vector_t *v1, const ri_vector_t *v2)
{
	...
	return (ri_vector_length3(&cross) * 0.5);
}
