	...
	ri_float_t imgflen )
{
        ...
        t = ( fdist - from->f[2] ) / ( to->f[2] - from->f[2] );
        dtheta = randomMT(  ) * 2.0 * M_PI;
        dr = sqrt( -log( randomMT(  ) ) / 5.0 ) * aperture;

        to->f[0] = t * ( to->f[0] - from->f[0] ) + from->f[0];
        to->f[1] = t * ( to->f[1] - from->f[1] ) + from->f[1];
        to->f[2] = fdist;
...
ri_camera_get_glmatrix( ri_matrix_t * m, const ri_camera_t * camera )
{
        ...
        ri_vector_normalize3( &right );
        ri_vector_normalize3( &up );
        ...
        ri_matrix_translate( m,
                             camera->cam_pos.f[0],
                             camera->cam_pos.f[1], camera->cam_pos.f[2] );
        ri_matrix_inverse( m );
}
