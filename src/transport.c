 ...
 */
int
ri_transport_sample(ri_render_t *render, const ri_ray_t *ray,
...
trace_path(ri_render_t *render, ri_ray_t *ray, ri_transport_info_t *result)
{
	...
	result->surfinfo.inside = 0;
	hit = ri_raytrace(render, ray, &(result->surfinfo));
	...
	if (hit) {
		...
		material = result->surfinfo.geom->material;
		if (!material) {
			d = 1.0;
			s = 0.0;
			t = 0.0;
		...
		if (s > 0.0) {
			...
			if (result->surfinfo.geom->material &&
			    result->surfinfo.geom->material->fresnel) {

				/* Fresnel reflection */
			
				ri_fresnel(&ray->dir, &transmit_ray,
					   &kr, &kt,
					   &ray->dir, &result->surfinfo.normal,
					   eta);
				...
				ri_reflect(&(ray->dir),
					   &ray->dir,
					   &result->surfinfo.normal);

				fresnel_factor = 1.0f;
			}

	
			ri_vector_copy(&(ray->org), &result->surfinfo.pos);

			ri_vector_copy(&col, &result->surfinfo.color);
		...
		if (d > 0.0) {
			...
			ri_shade(&rad, &ray->dir, light, ray, &(result->surfinfo));
		...
		if (t > 0.0) {
			...
			if (hasfresnel) {
				...
				if (result->surfinfo.inside) {
					printf("inside val = %d\n", result->surfinfo.inside);
					printf("inside\n");
			...
			ri_vector_copy(&rayorg, &result->surfinfo.pos);
