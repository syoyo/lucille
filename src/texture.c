...
ri_texture_fetch(ri_vector_t *color,
		 const ri_texture_t *texture, double u, double v)
{
	int    i;
	int    idx;
	int    x, y;
		     ...
		     const ri_vector_t *dir)
{
	...
	/* +Y axis in world coordinate is
	 * -Y axis in texture coordinate.
	 * i.e. Y texture coordinate grows to bottom in world coordinate.
	 */
	if (ndir.e[2] >= -1.0 && ndir.e[2] < 1.0) {
		r = (1.0 / pi) * acos(ndir.e[2]);
	} else {
		r = 0.0;
	}

	norm2 = ndir.e[0] * ndir.e[0] + ndir.e[1] * ndir.e[1];
	if (norm2 > 1.0e-6) {
		r /= sqrt(norm2);

		/* sqrt(x) = (1/sqrt(x)) * (x) */
		//r /= rsqrt((float)norm2) * (float)norm2;
	}
...
void
ri_texture_scale(ri_texture_t *texture, float scale)
{
	int i;
...
load_jpeg(FILE *fp)
{
	...
	for (i = 0; i < width * height; i++) {
		switch (compos) {
		case 1:
			tex->data[4 * i + 0] = ((float)img[i] + 0.5) * w;
			tex->data[4 * i + 1] = ((float)img[i] + 0.5) * w;
			tex->data[4 * i + 2] = ((float)img[i] + 0.5) * w;
			tex->data[4 * i + 3] = 1.0f;
			break;

		case 2:
			tex->data[4 * i + 0] = ((float)img[2*i+0] + 0.5) * w;
			tex->data[4 * i + 1] = ((float)img[2*i+1] + 0.5) * w;
			tex->data[4 * i + 2] = 0.0f;
			tex->data[4 * i + 3] = 1.0f;
			break;
	
		case 3:
			tex->data[4 * i + 0] = ((float)img[3*i+0] + 0.5) * w;
			tex->data[4 * i + 1] = ((float)img[3*i+1] + 0.5) * w;
			tex->data[4 * i + 2] = ((float)img[3*i+2] + 0.5) * w;
			tex->data[4 * i + 3] = 1.0f;
			break;
...
gen_blockedmipmap(const char *filename, ri_rawtexture_t *texture)
{
	...
	minsize = (w < h) ? w : h;
	miplevels = (int)ceil(log((double)minsize) / log(2.0));
...
write_blockedmipmap(const char *filename, blockedmipmap_t *blkmipmap)
{
	...
	size = sizeof(float) * TEXBLOCKSIZE * TEXBLOCKSIZE;
