...
ri_attribute_new()
{
	 ...
	 * Set default value.
	 */
	ri_vector_set(&(attribute->color), one);
	ri_vector_set(&(attribute->opacity), one);
	...
	if (strcmp(token, "reflection") == 0) {
		...
		for (i = 0; i < n; i++) {
			if (strcmp(tokens[i], "diffuse") == 0) {
				...
				ri_vector_set(&(attr->material->kd), vec);	
			} else if (strcmp(tokens[i], "specular") == 0) {
				for (j = 0; j < 3; j++) {
					vec[j] = *((RtFloat *)params[i] + j);
				}
			
				ri_vector_set(&(attr->material->ks), vec);	
			} else if (strcmp(tokens[i], "transmittance") == 0) {
				for (j = 0; j < 3; j++) {
					vec[j] = *((RtFloat *)params[i] + j);
				}
			
				ri_vector_set(&(attr->material->kt), vec);	
...
ri_api_color(RtColor color)
{
	...
	ri_vector_set(&(attr->color), color);
}
...
ri_api_opacity(RtColor color)
{
	...
	ri_vector_set(&attr->opacity, color);
}
...
/* RiHider() implementation only handles "photon" photon mapping hider. */
void
ri_api_hider(RtToken type, RtInt n, RtToken tokens[], RtPointer params[])
{
	...
	if (strcmp(type, "photon") == 0) {
		/* renderer switches to use photon mapping */
		for (i = 0; i < n; i++) {
			if (strcmp(tokens[i], "emit") == 0) {
				pmapopt->nphotons = toInt(params[i]);
			}
		}
	}
}
