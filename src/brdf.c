...
static double fresnel(const ri_vector_t *v0, const ri_vector_t *v1);

double
ri_brdf_lambert(const ri_vector_t *wo, const ri_vector_t *wi,
		const ri_vector_t *n,
		double kd)
{
	double ldotn;
...
double
ri_brdf_blinn(const ri_vector_t *wo, const ri_vector_t *wi,
	      const ri_vector_t *n,
	      double kd, double ks, double glossness)
{
	ri_vector_t half;
	...
	return (kd + ks * pow(half.e[2], glossness));
}

double
ri_brdf_phong(const ri_vector_t *wo, const ri_vector_t *wi,
	      const ri_vector_t *n,
	      double kd, double ks,
	      double glossness)
{
	...
	rdotl = ri_vector_dot3(&r, wi);	/* (R . L)		*/
	ndotl = ri_vector_dot3(n, wi);	/* (N . L)		*/
 ...
 */
double
ri_brdf_modified_phong(const ri_vector_t *wo, const ri_vector_t *wi,
		       const ri_vector_t *n,
		       double kd, double ks,
		       double glossness)
{
	double      rdotl;
	double      diffuse, specular;	/* diffuse part and specular part */
 ...
 */
double
ri_brdf_ward_anisotropic(const ri_vector_t *wo, const ri_vector_t *wi,
			 const ri_vector_t *n,
			 const ri_vector_t *u, const ri_vector_t *v,
			 double kd, double ks,
			 double ax, double ay)
{
	ri_vector_t r;
	...
	double      costr;		/* cos(theta)_{r}	*/	
	double      costi;		/* cos(theta)_{i}	*/
	double      cospr;		/* cos(phi)_{r}		*/
	double      cospi;		/* cos(phi)_{i}		*/
	double      hnorm;		/* ||h||		*/
	double      hdotx, hdoty, hdotn;
	double      diffuse, specular;	/* diffuse part and specular part */
	double      c1, c2, c3;		/* for temporary	*/
	double      tand;
	...
	costr = ri_vector_dot3(&r, n);	/* cos(theta)_{r} = r . n	*/
	costi = ri_vector_dot3(wi, n);	/* cos(theta)_{i} = wi . n	*/
	if (costr <= 0.0) {
	...
	hdotn = ri_vector_dot3(&h, n);		/* h . n	*/
	hdotx = ri_vector_dot3(&h, u);		/* h . x	*/
	hdoty = ri_vector_dot3(&h, v);		/* h . y	*/
...
double
ri_brdf_ashikhmin_shirley(const ri_vector_t *wo, const ri_vector_t *wi,
			  const ri_vector_t *n,
			  const ri_vector_t *u, const ri_vector_t *v,
			  double kd, double ks, double nu, double nv)
{
	double      hk;
	double      nk1, nk2;
	double      fres;
	...
	hk  = ri_vector_dot3(&h, &k2);
	nk1 = ri_vector_dot3(&h, &k1);
	nk2 = ri_vector_dot3(&h, &k2);
	...
	hn = ri_vector_dot3(&h, n);
	hu = ri_vector_dot3(&h, u);
	hv = ri_vector_dot3(&h, v);
...
void
ri_brdf_ashikhmin_shirley_spf(ri_vector_t *specularray,
			      ...
			      double nu, double nv)
{
	...
	v.e[0] = (float)(cosp * sint);
	v.e[1] = (float)(sinp * sint);
	v.e[2] = (float)cost;
	...
	for (i = 0; i < 3; i++) {
		half.e[i] = basis[0].e[i] * v.e[0] +
			    basis[1].e[i] * v.e[1] +
			    basis[2].e[i] * v.e[2];
	}

	hn = ri_vector_dot3(&basis[2], &half);
	hu = ri_vector_dot3(&basis[0], &half);
	hv = ri_vector_dot3(&basis[1], &half);

	ri_vector_copy(&negdir, dir);
	ri_vector_neg(&negdir);
	neghn = ri_vector_dot3(&negdir, &half);
 ...
 */
void
ri_sample_modified_phong(ri_vector_t *wo,
			 double *pdf,
...
static double
fresnel(const ri_vector_t *v0, const ri_vector_t *v1)
{
	const double s = 0.1;
	double p;
	double one_minus_dot;

	one_minus_dot = ri_vector_dot3(v0, v1);
