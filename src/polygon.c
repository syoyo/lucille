...
ri_polygon_parse(RtInt nverts, RtInt n, RtToken tokens[], RtPointer params[])
{
	...
	for (i = 0; i < n; i++) {
		...
		if (strcmp(tokens[i], RI_P) == 0) { /* position */
			...
			if(!two_sided)
				ri_geom_add_positions(p, nverts, positions);
			else
				ri_geom_add_positions(p, nverts * 2, positions);
			...
			if(!two_sided)
				ri_geom_add_normals(p, nverts, normals);
			else /* it is two sided, so more normals... */
				ri_geom_add_normals(p, nverts * 2, normals);
			...
			for (j = 0; j < nverts; j++) {
				ri_vector_set(&(colors[j]), &param[3 * j]);
				if (two_sided) {
	...
	ri_geom_add_colors(p, nverts, colors);
	...
	ri_geom_add_opacities(p, nverts, opacities);
			...
			RtInt n, RtToken tokens[], RtPointer params[])
{
	...
	for (i = 0; i < (unsigned int)n; i++) {
		...
		if (strcmp(tokens[i], RI_P) == 0) { /* position */
			...
			if (attr->sides == 2) {
				ri_geom_add_positions(p, 2 * nvertices, positions);
			} else {
				ri_geom_add_positions(p, nvertices, positions);
			}
			...
			if (attr->sides == 2) {
				ri_geom_add_normals(p, 2 * nvertices, normals);
			} else {
				ri_geom_add_normals(p, nvertices, normals);
			}
		} else if (strcmp(tokens[i], RI_ST) == 0 ||
			   strcmp(tokens[i], "st") == 0) { /* tex coords */
			if (attr->sides == 2) {
	...
	ri_geom_add_colors(p, nvertices, colors);
	...
	ri_geom_add_opacities(p, nvertices, opacities);
...
ri_api_polygon(RtInt nverts, RtInt n, RtToken tokens[], RtPointer params[])
{
	...
	if (ri_render_get()->context->arealight_block) {
		arealight = (ri_light_t *)
			    ri_list_last(ri_render_get()->lightlist)->data;
		arealight->geom = geom;
	} else {
...
gen_basis(ri_geom_t *geom)
{
	...
	for (i = 0; i < geom->nindices / 3; i++) {
		...
		if (det == 0) {
			for (j = 0; j < 3; j++) {
				/* tmp.xyz = N.yzx */
				tmp.e[0] = geom->normals[id[j]].e[1];
				tmp.e[1] = geom->normals[id[j]].e[2];
				tmp.e[2] = geom->normals[id[j]].e[0];
			...
			dpdu.e[0] = (dx1 * dv2 - dv1 * dx2) * invdet;
			dpdu.e[1] = (dy1 * dv2 - dv1 * dy2) * invdet;
			dpdu.e[2] = (dz1 * dv2 - dv1 * dz2) * invdet;
	...
	for (i = 0; i < geom->npositions; i++) {
		...
		ri_vector_normalize(&(geom->tangents[i]));
		...
		ri_vector_cross3(&(geom->binormals[i]),
				 &(geom->tangents[i]),
				 &(geom->normals[i]));
		ri_vector_normalize(&(geom->binormals[i]));
