                     ...
                     float *x, float *y, float *z)
{
    ...
    for (i = 0, lambda = 380; lambda < 780.1; i+=2, lambda += 10) {
        float Me;
...
#endif
void spectrum_to_xyz(float *spectrum,
                     float *x, float *y, float *z)
{
    int i;
    float lambda, X = 0, Y = 0, Z = 0; // , XYZ;
    ...
    for (i = 0, lambda = 380; lambda < 780.1; i++, lambda += 5) {
        ...
        X += spectrum[i / 2] * cie_colour_match[i][0];
        Y += spectrum[i / 2] * cie_colour_match[i][1];
        Z += spectrum[i / 2] * cie_colour_match[i][2];

    }

    // Modified that do not normalize the result. 
    /*
    XYZ = (X + Y + Z);
    if (fabs(XYZ) > 1.0e-48) { 
        *x = X / XYZ;
