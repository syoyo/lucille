	...
	const ri_float_t  glossness)
{
	...
	return (kd + ks * pow(half.f[2], glossness));
}
...
ri_float_t
ri_brdf_ashikhmin_shirley(const ri_vector_t *wo,
			  const ri_vector_t *wi,
			  const ri_vector_t *n,
			  const ri_vector_t *u,
			  const ri_vector_t *v,
			  ...
			  const ri_float_t   nv)
{
	...
	ri_vector_normalize3(&k1);
	ri_vector_normalize3(&k2);
	...
	const ri_float_t   nv)
{
	...
	v.f[0] = (ri_float_t)(cosp * sint);
	v.f[1] = (ri_float_t)(sinp * sint);
	v.f[2] = (ri_float_t)cost;

	ri_ortho_basis(basis, dir);

	/* transform local half vector into surface space */
	for (i = 0; i < 3; i++) {
		half.f[i] = basis[0].f[i] * v.f[0] +
			    basis[1].f[i] * v.f[1] +
			    basis[2].f[i] * v.f[2];
	}
