		  ...
		  const char *name, const void *val)
{
	...
	if (p->type == TYPESTRING) {
		...
		if (p->type == TYPEFLOAT) {
			...
			v.f[0] = ((ri_float_t *)val)[0];
			v.f[1] = ((ri_float_t *)val)[1];
			v.f[2] = ((ri_float_t *)val)[2];
			v.f[3] = 1.0;
...
diffuse(const ri_status_t *status, ri_color_t *dst, const ri_vector_t *N)
{
	...
	ri_vector_normalize3(&light);

	dot = ri_vector_dot3(N, &light);
	...
	if (dot <= 0.0) {
		...
		dst->f[0] = (ri_float_t)dot;
		dst->f[1] = (ri_float_t)dot;
		dst->f[2] = (ri_float_t)dot;
	}
}
	 ...
	 ri_float_t roughness)
{
	...
	ri_vector_normalize3(&half);

	dot = ri_vector_dot3(N, &half);
	...
	ri_vector_set3(dst, coeff, coeff, coeff);
}
...
void
refract(ri_vector_t *dst, const ri_vector_t *I, const ri_vector_t *N, ri_float_t eta)
{
            ...
            ri_color_t *dst, const char *name, const ri_vector_t *coords)
{
	...
	m = 2.0 * sqrt(coords->f[0] * coords->f[0] +
		       coords->f[1] * coords->f[1] +
		       (coords->f[2] + 1.0) * (coords->f[2] + 1.0));

	
	if (m != 0.0) {
		tex_coords.f[0] = coords->f[0] / m + 0.5;
		tex_coords.f[1] = coords->f[1] / m + 0.5;
	} else {
		tex_coords.f[0] = 0.5;
		tex_coords.f[1] = 0.5;
	}
	       ...
	       ri_color_t *dst, const char *name, const ri_vector_t *coords)
{
	...
	u = (int)(texwidth * coords->f[0]) % texwidth;
	v = (int)(texwidth * coords->f[1]) % texwidth;
	...
	c = ((u & step) == 0) ^ ((v & step) == 0);
	dst->f[0] = (ri_float_t)c;
	dst->f[1] = (ri_float_t)c;
	dst->f[2] = (ri_float_t)c;
          ...
          const ri_vector_t *P, const ri_vector_t *N, ri_float_t nsamples)
{
	...
	for (j = 0; j < nphi; j++) {
		for (i = 0; i < (int)ntheta; i++) {
			...
			for (k = 0; k < 3; k++) {
				ray.dir.f[k] = dir.f[0] * basis[0].f[k]
					     + dir.f[1] * basis[1].f[k]		
					     + dir.f[2] * basis[2].f[k];
			}
			...
			// the ray direction
			ray.org.f[0] += 0.0001 * ray.dir.f[0]; 
			ray.org.f[1] += 0.0001 * ray.dir.f[1]; 
			ray.org.f[2] += 0.0001 * ray.dir.f[2]; 
...
ri_float_t
inversesqrt(ri_float_t x)
{
	if (x < 0.0) return 1.0;

	if (x < 0.001) return 1.0;

	return 1.0f / (ri_float_t)sqrt(x);
}

ri_float_t
mod(ri_float_t a, ri_float_t b)
{
	int   n;
	ri_float_t c;
...
ri_float_t
minf(ri_float_t a, ri_float_t b)
{
	if (a < b) return a;
...
ri_float_t
clampf(ri_float_t a, ri_float_t min, ri_float_t max)
{
	if (a < min) return min;
	if (a > max) return max;
...
ri_float_t
step(ri_float_t min, ri_float_t value)
{
...
ri_float_t
smoothstep(ri_float_t min, ri_float_t max, ri_float_t value)
{
	double t;
...
void
mixv(ri_vector_t *dst, const ri_vector_t *x, const ri_vector_t *y, ri_float_t alpha)
{
	dst->f[0] = (1.0 - alpha) * x->f[0] + alpha * y->f[0];
	dst->f[1] = (1.0 - alpha) * x->f[1] + alpha * y->f[1];
	dst->f[2] = (1.0 - alpha) * x->f[2] + alpha * y->f[2];
}
	...
	ri_float_t         angle)
{
	...
	ri_vector_copy(&ray.org, P);
	ray.org.f[0] += N->f[0] * 0.0001;	
	ray.org.f[1] += N->f[1] * 0.0001;	
	ray.org.f[2] += N->f[2] * 0.0001;	
	...
	ri_float_t angle)
{
	...
	for (j = 0; j < nphi; j++) {
		for (i = 0; i < ntheta; i++) {
			...
			//assert(!isnan(dir.f[0]));
			//assert(!isnan(dir.f[1]));
			//assert(!isnan(dir.f[2]));

			for (k = 0; k < 3; k++) {
				ldir.f[k] = dir.f[0] * glightinfo[tid].basis[0].f[k]
				          + dir.f[1] * glightinfo[tid].basis[1].f[k]
				          + dir.f[2] * glightinfo[tid].basis[2].f[k];
			}
			ldir.f[3] = 0.0;

			ri_vector_normalize3(&ldir);
			...
			if (isnan(glightinfo[tid].samples[count].Cl.f[0])) {
				printf("ldir = ");
