...
ri_api_world_end()
{
	...
	case ACCEL_OCTREE_FRISKEN:
		ri_render_get()->octree = ri_accel_build_octree();
		break;
	case ACCEL_GRID:
		ri_render_get()->accel_grid = ri_accel_build_uniform_grid();
		break;
	case ACCEL_BSP:
		ri_render_get()->accel_bsp = ri_accel_build_bsp();
		break;
	case ACCEL_KDTREE:
		ri_render_get()->accel_kdtree = ri_accel_build_kdtree();
		break;
	default:
		ri_render_get()->accel_grid = ri_accel_build_uniform_grid();
		break;
