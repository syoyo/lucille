/* Beta-0.3.0: MOSAIC default surface shader for Blender integration */

#include "MOSAICfunctions.h"

surface
MOSAICsurface(
	float Amb=0.5; //Standard Blender material controls
	float Ref=0.8;
	float Spec=0.5;
	float RayMir=1;
	float Hard=12.5;
	...
	float isShadeless=0;
	float isOcclusion=0;
	color DifCol=0.8; //Standard Blender material colors
	color SpecCol=1;
	color MirCol=1;
	color OccCol=1;
	color AmbCol=0;
	...
	varying float Ms=-1;)
{
	if (SetMode == 0 || Ms == -1 || SetIndex == Ms) //Only shade a surface belonging to the current MaterialSet
	{
		...
		if (ColMap != "") //Are we using a color texture map?
		{
			...
			Ci = color mix(DifCol,color texture(ColMap,sAdj,tAdj),ColMix);
		}
		else Ci = Cs*DifCol;
		if (isShadeless == 0) //Only process the rest if we are not shadeless
		{
			...
			if (IBLMap != "" || EnvMap != "" || isRayMirror > 0 || isRayTransp > 0)
			{
				color Umc = MirCol;
				float Um = RayMir;
				...
				if (IBLMap != "" || EnvMap != "" || isRayMirror > 0 || isRayTransp > 0) //Are we using reflection/refraction?
				{
					vector R,T;
					...
					if (EnvMap != "" || (IBLMap != "" && isRayMirror == 0 && isRayTransp == 0)) //Is there a env map or a ibl map and not raytracing?
					{
						uniform string mapName = "";
						if (EnvMap != "") mapName = EnvMap;
						else mapName = IBLMap;
						uniform string texType = "";
						textureinfo(mapName,"type",texType);
						if (texType == "texture" || texType == "Plain Texture")
						{
							point Pndc = transform("NDC",P);
							float tx = xcomp(Pndc);
							float ty = ycomp(Pndc);
							if (Kmr > 0) Ci = Ci*(1-Um*Kmr)+color texture(mapName,tx,ty,"blur",EnvBlur)*Umc*Um*Kmr;
						}
						else
						{
							T = ntransform("world",normal(T))*(1,1,-1);
							R = ntransform("world",normal(R))*(1,1,-1);
							if (Kmt > 0) Ci = Ci*Kmt+color environment(mapName,T,"samples",EnvSamples,"blur",EnvBlur)*(1-Kmt);
							if (Kmr > 0) Ci = Ci*(1-Um*Kmr)+color environment(mapName,R,"samples",EnvSamples,"blur",EnvBlur)*Umc*Um*Kmr;
						}
					}
					else
					{
		...
		if (AlphaMap != "") //Are we using a alpha texture map?
		{
			...
			Oi = Os*mix(float texture(AlphaMap,sAdj,tAdj),Alpha,Alpha);
		}
		else Oi = Os*Alpha;
		Ci *= Oi;
	}
